...
class TestDecoding:
    ...
    def test_simple_message_decoding(self):
        """A packet without the special escape sequences
        should result in a message without special bytes."""
        msg = b"hallo"
        packet = END + msg + END
    ...
    def test_special_character_decoding(self, packet, msg):
        """A packet with special escape sequences should result
        in a message with the appropriate special bytes."""
...
class TestDriver:
    ...
    def test_split_message_decoding(self):
        """Test that receives only returns the message after
        the complete packet has been received.

        The message contains a NULL byte to test the correct
        handling of this when single bytes are received."""
        msg = b"hallo\0bye"
        packet = END + msg
        for byte_ in packet:
    ...
    def test_flush_buffers_with_empty_packet(self):
        """Test that receiving an empty byte string results in
        completion of the pending packet."""
